...
(function(){
  ...
  function applyPreviousClosingAsCanonical(cfg,prev){
    ...
    try{
      if(typeof openingPrevModal!=='undefined'&&Array.isArray(openingPrevModal)){
        openingPrevModal.forEach((x,ix)=>{
          ...
          x.value=amount;
          const check=document.getElementById('prevModalCheck'+(ix+1));
          if(check)check.value=amount.toLocaleString('id-ID');
          const row=check?.closest('tr');
          if(row?.cells?.[1])row.cells[1].textContent=money(amount);
          const reason=document.getElementById('prevModalReason'+(ix+1));if(reason)reason.value='';
        });
      }
    }catch(_){}

    // Paket: STOK AKHIR hari sebelumnya benar-benar menjadi STOK SISTEM hari baru.
    try{
      if(typeof pkgCatalog!=='undefined'&&Array.isArray(pkgCatalog)&&Array.isArray(cats.pkg)){
        ...
        pkgCatalog.forEach((p,ix)=>{
          ...
          if(typeof openingPkg!=='undefined'&&Array.isArray(openingPkg)&&openingPkg[ix]){
            openingPkg[ix].stock=stock;
            openingPkg[ix].base=base;
            openingPkg[ix].sell=sell;
          }
    ...
    // Rokok: Display akhir + Gudang akhir menjadi STOK SISTEM opening hari baru.
    try{
      if(typeof cigCatalog!=='undefined'&&Array.isArray(cigCatalog)&&Array.isArray(cats.cig)){
        ...
        cigCatalog.forEach((x,ix)=>{
          ...
          if(typeof openingCig!=='undefined'&&Array.isArray(openingCig)&&openingCig[ix]){
            openingCig[ix].display=display;
            openingCig[ix].warehouse=warehouse;
            openingCig[ix].base=base;
            openingCig[ix].sell=sell;
          }
    ...
    try{
      if(typeof updateOpeningCorrections==='function')updateOpeningCorrections();
    }catch(_){}
    return true;
  }
